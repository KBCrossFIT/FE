      ...
      <ul class="nav-aside">
          <li class="menu1" @click="toggleDropdown(1)">
              <a href="javascript:void(0)">
                  <i class="fas fa-briefcase"></i> <!-- Portfolio Icon -->
                      ...
                      <table class="portfolio-table">
                          ...
                          <tbody>
                              <tr v-for="(portfolio, index) in portfolios" :key="index">
                                  ...
                                  <td :class="{'positive': portfolio.returns > 0, 'negative': portfolio.returns < 0}">
                                      {{ portfolio.returns }}%
                                  </td>
          ...
          <li class="menu3" @click="toggleDropdown(3)">
              <a href="javascript:void(0)">
                  <i class="fas fa-history"></i> <!-- Recent Products Icon -->
...
export default {
  ...
  setup() {
      ...
      const activeDropdown = ref(null); // 활성화된 드롭다운 메뉴의 번호
      const router = useRouter(); // Set up the router

      const toggleDropdown = (menuNumber) => {
          if (activeDropdown.value === menuNumber) {
              activeDropdown.value = null; // 이미 활성화된 메뉴 클릭 시 닫음
          } else {
              activeDropdown.value = menuNumber; // 선택한 메뉴의 드롭다운 활성화
          }
      };

      const goToCreatePortfolio = () => {
          console.log('포트폴리오 구성하기 버튼 클릭됨');
          router.push('/make-portfolio'); // Navigate to create portfolio page
      };
