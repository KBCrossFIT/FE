    ...
    <div class="products-wrapper">
      <div class="products-container">
        <div class="products-box">
          ...
          <table class="products-table">
            <thead>
              <tr>
                ...
                <th v-if="selectedTab === '예금' || selectedTab === '적금'">금융회사명</th>
                <th v-if="selectedTab === '예금' || selectedTab === '적금'">기본금리</th>
                <th v-if="selectedTab === '예금' || selectedTab === '적금'">최고금리</th>
            ...
            <tbody>
              ...
              <template v-if="selectedTab === '예금' || selectedTab === '적금'">
                ...
                <tr v-if="displayedProducts && displayedProducts.products" v-for="product in displayedProducts.products" :key="product.productId">
                  <td>{{ product.finPrdtNm }}</td>
                  <td>{{ product.korCoNm }}</td>
                  <!-- 기본금리와 최고금리 출력 (저축 기간 12개월 기준) -->
                  <td>{{ getRateByTerm(displayedProducts.rates, product.productId, 12)?.intrRate }}%</td>
                  <td>{{ getRateByTerm(displayedProducts.rates, product.productId, 12)?.intrRate2 }}%</td>
                  <td>
                    <v-btn
                      icon
                      :class="{ 'in-cart': cart.includes(product.productId) }"
                      @click="toggleCart(product.productId)"
                      :style="cart.includes(product.productId) ? 'background-color: #4caf50; color: white;' : ''"
                    >
          ...
          <div v-if="displayedProducts.length === 0" class="no-products">상품이 없습니다.</div>
        </div>
      </div>
    </div>
  </template>
