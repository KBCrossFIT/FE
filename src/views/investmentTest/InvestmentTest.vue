...
export default {
  data() {
    return {
      questions: [
        ...
        {
          question:
            '5. 전체 자산(부동산 포함) 중 금융자산 비중이 어떻게 되십니까?',
  ...
  methods: {
    selectAnswer(option, index) {
      ...
      // 선택된 답변에 해당하는 점수를 저장
      this.selectedScores[this.currentQuestionIndex] =
    ...
    finishTest() {
      // 총 점수 계산 (선택된 점수의 합산)
      this.totalScore = this.selectedScores.reduce(
        (acc, score) => acc + score,
        0
      );
...
.InvestmentTest-container {
  ...
  justify-content: space-between; /* 공간을 위 아래로 분리 */
  height: auto;
  padding: 40px;
...
.navigation {
  ...
  gap: 20px;
  width: 20%;
  max-width: 700px; /* 질문 영역과 동일한 최대 너비로 설정 */
  margin: 0 auto;
}
...
.navigation-End-btn {
  ...
  gap: 10px;
  margin-top: auto; /* 상단의 요소들과 간격을 띄움 */
}
</style>
