              ...
              <tr v-for="item in filteredProducts" :key="item.productId">
                ...
                <template v-if="item.productType === 'S'">
                  ...
                  <td>
                    ...
                    기본 금리: {{ item.rates[0].intrRate }}%<br/>
                    최고 금리: {{ item.rates[0].intrRate2 }}%
                  </td>
                </template>
                ...
                <template v-if="item.productType === 'F'">
                  ...
                  <td>
                    ...
                    자산운용사명: {{ item.productNm }}<br/>
                    1개월 수익률: {{ item.yield1 }}%<br/>
                    3개월 수익률: {{ item.yield3 }}%<br/>
                    6개월 수익률: {{ item.yield6 }}%<br/>
                    12개월 수익률: {{ item.yield12 }}%<br/>
                    상품등급: {{ item.riskLevel }}<br/>
                    선취수수료율: {{ item.advancedFee }}%<br/>
                    총보수율: {{ item.totalPayoffRate }}%
                  </td>
                </template>
                ...
                <template v-if="item.productType === 'B'">
                  ...
                  <td>
                    ...
                    한국신용평가: {{ item.kbpScrsItmsKcdNm }}<br/>
                    NICE평가정보: {{ item.niceScrsItmsKcdNm }}<br/>
                ...
                <td>
                  <input type="number" v-model.number="item.investmentAmount" min="0" placeholder="투자액 입력"
                         class="styled-input"/>
                </td>
                <td>
  ...
  setup() {
    ...
    const filteredProducts = computed(() => {
      return selectedCategory.value
          ? selectedProducts.value.filter(product => product.productType === selectedCategory.value)
      ...
      const formattedProducts = selectedProducts.value
          ...
          .map(product => {
            ...
            if (product.productType === 'S') {
              info.rsrvType = product.rsrvType;
              info.saveTerm = product.saveTrm;
              info.intrRate = product.intrRate;
    ...
    // 저장 함수
    const savePortfolio = () => {
      const portfolioData = portfolioStore.postPortfolioAction(formatDataForSave(), "name1");
